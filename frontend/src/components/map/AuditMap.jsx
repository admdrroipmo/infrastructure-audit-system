  ...
  useMapEvents({
    click: async (e) => {
      ...
      try {
        ...
        if (cityName) {
          const psgcResponse = await fetch(
            `http://localhost:5000/api/v1/psgc/match/${encodeURIComponent(cityName)}`,
          );
          console.log("📍 PSGC match response status:", psgcResponse.status);

          if (psgcResponse.ok) {
            const psgcData = await psgcResponse.json();
            console.log("📍 PSGC match data:", psgcData);

            const fullLocation = data.display_name || "";
            const region = psgcData.region || "";
            const province = psgcData.province || "";
            const city = psgcData.city || psgcData.name || "";
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (cityName) {
          ...
          if (psgcResponse.ok) {
            const psgcData = await psgcResponse.json();
            console.log("🔍 PSGC match result:", psgcData);
            const fullLocation = result.display_name || "";
            const region =
              psgcData.parent?.parent?.name || psgcData.parent?.name || "";
            const province = psgcData.parent?.name || "";
            const city = psgcData.name || "";
